...
<?php
    $data_url = Yii::app()->createUrl('city/listjson');
    $edit_url = Yii::app()->createUrl('city/edit');
    $delete_url = Yii::app()->createUrl('city/delete');
    ...
    var table = $('#city_table').DataTable({
            ...
            'columnDefs': [
                ...
                // In row edit button
                {
                    ...
                    "data": null,
                    'render': function ( data, type, row ) {
                        var editurl = "<a class='editrow btn btn-info btn-xs' href='{$edit_url}/city_id/" + row[0] + "'>Edit</a>";
                        // alert(editurl);
                        return editurl;
                    },
                },
        ...
        $('#city_table tbody').on( 'click', 'button', function () {
          
            var className = this.className;
          
            var data = table.row( $(this).parents('tr') ).data();
          
            var result = window.confirm("Do you really want to Delete the city record for " + data[1] + "?");
            if (result == false) {
                e.preventDefault();
                return false;
            }

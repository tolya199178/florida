...
<style type="text/css">
    ...
    <nav class="navbar navbar-default" role="navigation">
        <div class="navbar-header">
            <a class="navbar-brand" href="#">Manage Activity Type</a>
        </div>
        <div>
            <div class="navbar-form navbar-left" role="search">
                <div class="form-group">
                    <a
                        href="<?php echo Yii::app()->createUrl('activitytype/create'); ?>"
                        class="btn btn-primary">Add New Activity Type</a>
                </div>
            </div>
    ...
    <table id="activity_type_table" class="table table-striped table-bordered"
        cellspacing="0" width="100%">
        <thead>
            <tr>
                <th>Activity Type ID</th>
                <th>Activity Type</th>
                <th>Activity Type  Keyword</th>
                <th>Related Words</th>
                <th>&nbsp;</th>
                <th>&nbsp;</th>
...
<?php
    $data_url = Yii::app()->createUrl('activitytype/listjson');
    $edit_url = Yii::app()->createUrl('activitytype/edit');
    $delete_url = Yii::app()->createUrl('activitytype/delete');

$script = <<<EOD
    var table = $('#activity_type_table').DataTable({
            ...
            'columnDefs': [
                ...
                // In row edit button
                {
                    ...
                    "data": null,
                    'render': function ( data, type, row ) {
                        var editurl = "<a class='editrow btn btn-info btn-xs' href='{$edit_url}/activity_type_id/" + row[0] + "'>Edit</a>";
                        // alert(editurl);
                        return editurl;
                    },
                },
        ...
        $('#activity_type_table tbody').on( 'click', 'button', function () {

            var className = this.className;

            var data = table.row( $(this).parents('tr') ).data();

            var result = window.confirm("Do you really want to Delete the activity type record for " + data[1] + "?");
            if (result == false) {
                e.preventDefault();
                return false;
            }
        ...
        return true;
          ...
          if (className == "editrow") {
            ...
            //  $('#myModal').modal('show');
            url = "{$edit_url}/activity_type_id/" + data[0];
            $('#myModal').removeData();
                $('#myModal').modal({
                    remote : url
                });

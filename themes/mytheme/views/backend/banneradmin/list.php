...
<style type="text/css">
    ...
    <nav class="navbar navbar-default" role="navigation">
        <div class="navbar-header">
            <a class="navbar-brand" href="#">Manage Banners</a>
        </div>
        <div>
            <div class="navbar-form navbar-left" role="search">
                <div class="form-group">
                    <a
                        href="<?php echo Yii::app()->createUrl('banneradmin/create'); ?>"
                        class="btn btn-primary">Add New Banner</a>
                </div>
            </div>
...
<?php
    $data_url = Yii::app()->createUrl('banneradmin/listjson');
    $edit_url = Yii::app()->createUrl('banneradmin/edit');
    $delete_url = Yii::app()->createUrl('banneradmin/delete');
    ...
    var table = $('#banner_table').DataTable({
            ...
            'columnDefs': [
                ...
                // In row edit button
                {
                    ...
                    "data": null,
                    'render': function ( data, type, row ) {
                        var editurl = "<a class='editrow btn btn-info btn-xs' href='{$edit_url}/banner_id/" + row[0] + "'>Edit</a>";
                        // alert(editurl);
                        return editurl;
                    },
                },
        ...
        $('#banner_table tbody').on( 'click', 'button', function () {

            var className = this.className;

            var data = table.row( $(this).parents('tr') ).data();

            var result = window.confirm("Do you really want to Delete the banner record for " + data[1] + "?");
            if (result == false) {
                e.prbannerDefault();
                return false;
            }

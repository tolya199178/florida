...
<nav class="navbar navbar-default" role="navigation">
    <div class="navbar-header">
        <a class="navbar-brand" href="#">Manage Questionnaire</a>
    </div>
    <div>
        <div class="navbar-form navbar-left" role="search">
            <div class="form-group">
                <a
                    href="<?php echo Yii::app()->createUrl('questionnaire/create'); ?>"
                    class="btn btn-primary">Add New Questionnaire</a>
            </div>
        </div>
...
<table id="questionnaire_table" class="table table-striped table-bordered"
...
<?php
$data_url = Yii::app()->createUrl('questionnaire/listjson');
$edit_url = Yii::app()->createUrl('questionnaire/edit');
$delete_url = Yii::app()->createUrl('questionnaire/delete');

$script = <<<EOD
    var table = $('#questionnaire_table').DataTable({
            ...
            'columnDefs': [
                ...
                // In row edit button
                {
                    ...
                    "data": null,
                    'render': function ( data, type, row ) {
                        var editurl = "<a class='editrow btn btn-info btn-xs' href='{$edit_url}/questionnaire_id/" + row[0] + "'>Edit</a>";
                        // alert(editurl);
                        return editurl;
                    },
                },
        ...
        $('#questionnaire_table tbody').on( 'click', 'button', function () {

            var className = this.className;

            var data = table.row( $(this).parents('tr') ).data();

            var result = window.confirm("Do you really want to Delete the questionnaire record for " + data[1] + "?");
            if (result == false) {
                e.preventDefault();
                return false;
            }

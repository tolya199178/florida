...
<style type="text/css">
    ...
    <nav class="navbar navbar-default" role="navigation">
        <div class="navbar-header">
            <a class="navbar-brand" href="#">Manage Activities</a>
        </div>
        <div>
            <div class="navbar-form navbar-left" role="search">
                <div class="form-group">
                    <a
                        href="<?php echo Yii::app()->createUrl('activity/create'); ?>"
                        class="btn btn-primary">Add New Activity</a>
                </div>
            </div>
...
<?php
    $data_url = Yii::app()->createUrl('activity/listjson');
    $edit_url = Yii::app()->createUrl('activity/edit');
    $delete_url = Yii::app()->createUrl('activity/delete');
    ...
    var table = $('#activity_table').DataTable({
            ...
            'columnDefs': [
                ...
                // In row edit button
                {
                    ...
                    "data": null,
                    'render': function ( data, type, row ) {
                        var editurl = "<a class='editrow btn btn-info btn-xs' href='{$edit_url}/activity_id/" + row[0] + "'>Edit</a>";
                        // alert(editurl);
                        return editurl;
                    },
                },
        ...
        $('#activity_table tbody').on( 'click', 'button', function () {
            ...
            var result = window.confirm("Do you really want to Delete the activity record for " + data[1] + "?");
            if (result == false) {
                e.preventDefault();
                return false;
            }
        ...
        return true;
          ...
          if (className == "editrow") {
            ...
            //  $('#myModal').modal('show');
            url = "{$edit_url}/activity_id/" + data[0];
            $('#myModal').removeData();
                $('#myModal').modal({
                    remote : url
                });

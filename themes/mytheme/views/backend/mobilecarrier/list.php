...
<style type="text/css">
    ...
    <nav class="navbar navbar-default" role="navigation">
        <div class="navbar-header">
            <a class="navbar-brand" href="#">Manage Mobile Carriers</a>
        </div>
        <div>
            <div class="navbar-form navbar-left" role="search">
                <div class="form-group">
                    <a
                        href="<?php echo Yii::app()->createUrl('mobilecarrier/create'); ?>"
                        class="btn btn-primary">Add New Mobile Carrier</a>
                </div>
            </div>
    ...
    <table id="mobile_carrier_table" class="table table-striped table-bordered"
        cellspacing="0" width="100%">
        <thead>
            <tr>
                <th>Mobile Carrier ID</th>
                <th>Carrier Name</th>
                <th>Sending Allowed</th>
                <th>Recipient</th>
                <th>&nbsp;</th>
                <th>&nbsp;</th>
...
<?php
    $data_url   = Yii::app()->createUrl('mobilecarrier/listjson');
    $edit_url   = Yii::app()->createUrl('mobilecarrier/edit');
    $delete_url = Yii::app()->createUrl('mobilecarrier/delete');

$script = <<<EOD
    var table = $('#mobile_carrier_table').DataTable({
            ...
            'columnDefs': [
                ...
                // In row edit button
                {
                    ...
                    "data": null,
                    'render': function ( data, type, row ) {
                        var editurl = "<a class='editrow btn btn-info btn-xs' href='{$edit_url}/mobile_carrier_id/" + row[0] + "'>Edit</a>";
                        // alert(editurl);
                        return editurl;
                    },
                },
        ...
        $('#mobile_carrier_table tbody').on( 'click', 'button', function () {

            var className = this.className;

            var data = table.row( $(this).parents('tr') ).data();

            var result = window.confirm("Do you really want to Delete the mobile carrier record for " + data[1] + "?");
            if (result == false) {
                e.preventDefault();
                return false;
            }
        ...
        return true;
          ...
          if (className == "editrow") {
            ...
            //  $('#myModal').modal('show');
            url = "{$edit_url}/mobile_carrier_id/" + data[0];
            $('#myModal').removeData();
                $('#myModal').modal({
                    remote : url
                });
